...
var grapple_mode : AoeGrapple.GrappleMode = AoeGrapple.GrappleMode.REEL
...
func setup() -> void:
	...
	_aoe_grapple.grapple_mode = grapple_mode
...
func add_node_to_nearby_list(new_node : Node3D):
	if new_node == character or _aoe_grapple.get_can_grapple_node(new_node) == false or new_node is BaseCharacter:
		return
...
func physics_process(delta: float) -> void:
	if show_closest_indicator == true:
		_closest_node_tracker.scan_origin = character.global_position
...
func action() -> void:
	...
	if grapple_mode == AoeGrapple.GrappleMode.REEL:
