...
const LENS_FLARE_SUN_FLARE : Texture2D = preload("res://Assets/Textures/VFX/lens_flare.png")
const CORE_LENS_FLARE_SUN_FLARE : Texture2D = preload("res://Assets/Textures/VFX/Flare_losless.png")
const GLSL_FILE : RDShaderFile = preload("res://Shaders/compute/flare.glsl")
var lens_flare_tex: RID
var corelens_flare_tex: RID
...
func _notification(what: int) -> void:
	...
	if what == NOTIFICATION_PREDELETE:
		...
		if corelens_flare_tex.is_valid(): rd.free_rid(corelens_flare_tex)
...
func _render_callback(_effect_callback_type: int, render_data: RenderData):
	...
	push_constants.append(GlobalLight.sun_screen_position.x)
	push_constants.append(GlobalLight.sun_screen_position.y)
	push_constants.append(GlobalLight.sun_dot)
	...
	for view in scene_buffers.get_view_count():
		...
		rd.compute_list_bind_uniform_set(compute_list, core_lens_flare_texture_set, 2)
...
func _init_compute_shader() -> void:
	...
	var core_flare_image : Image = CORE_LENS_FLARE_SUN_FLARE.get_image()
	core_flare_image.decompress()
	core_flare_image.convert(Image.FORMAT_RGF)
	
	var core_texture_format : RDTextureFormat = RDTextureFormat.new()
	core_texture_format.width = core_flare_image.get_width()
	core_texture_format.height = core_flare_image.get_height()
	core_texture_format.format = RenderingDevice.DATA_FORMAT_R32G32_SFLOAT;
	core_texture_format.usage_bits = RenderingDevice.TEXTURE_USAGE_SAMPLING_BIT;
	corelens_flare_tex = rd.texture_create(core_texture_format, RDTextureView.new(), [core_flare_image.get_data()])
