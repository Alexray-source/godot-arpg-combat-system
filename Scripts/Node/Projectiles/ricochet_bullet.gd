...
func ricochet(intersected_hurtbox : HurtBox):
	...
	if target_characters == true:
		nearby_hurtboxes = nearby_hurtboxes.filter(filter_for_character)
	else:
	...
	var direction = intersected_hurtbox.global_position.direction_to(random_hurtbox.global_position)
